...
{
    public partial class BackStorageForm : Form
    {
        /// <summary>
        /// 数据库连接句柄
        /// </summary>
        ...
        /// 出库表实体类
        /// </summary>
        EntityOutput_storage output_storageEntity = null;
        /// <summary>
        /// 不合格表实体类
        /// </summary>
        ...
        private void BandingDgvCounter()
        {
            try
            {
                ...
                //获取绑定数据表
                DataTable dtInput_storage = getData.GetTableBySqlStr(Constants.SqlStr.TC_INPUT_INIT_STORAGE_LEFTJOIN_GOODS_LEFTJOIN_TC_OUTPUT_STORAGE_LEFTJOIN_TC_CUSTOMER, sp);
                //添加序列号
                int countNumber = 0;
        ...
        private void setButtonState()
        {
            ...
            switch (dataType)
            {
                case DataType.None:
                    ...
                    labelMaxCOUNT.Visible = false;
                    dataTime_input_maketime.Enabled = false;
                    dateTime_input_instorage_date.Enabled = false;
                    dateTime_backstorage_date.Enabled = false;
                    dateTimePicker_CheckTime.Enabled = false;
                    ...
                    textBox_input_check_persion.ReadOnly = true;

                    textBox_output_code.BackColor = Color.FromArgb(236, 233, 216);
                ...
                case DataType.Insert:
                    ...
                    //dateTime_backstorage_date.Enabled = true;
                    //textBox_input_quality_reg.ReadOnly = false;
                    dateTimePicker_CheckTime.Enabled = true;
                    ...
                    textBox_output_code.Text = "双击选择一个产品名称...";
                    textBox_output_code.BackColor = Color.FromArgb(255, 255, 255);
                    ...
                    dateTime_backstorage_date.Value = DateTime.Now;
        ...
        public BackStorageForm()
        {
            editFlag = 0;
            InitializeComponent();
            this.dataGridView_input_storage.AlternatingRowsDefaultCellStyle.BackColor = Color.FromArgb(239, 243, 251);
            dateTime_input_instorage_date_from.Enabled = false;
            dateTime_input_instorage_date_to.Enabled = false;
        }
        //*******************************************************************
        /// <summary>
        /// 双击文本框弹出不同的选择窗体
                ...
                case "textBox_output_code":
                    {
                        ...
                        if (dataType == DataType.Insert)
                        {
                            ...
                            child.ShowDialog();
                            //获取销售信息
                            if (!child.outputCode.Equals(string.Empty))
                            {
                                supplierName = child.supplierName;
                                outputCode = child.outputCode;
                                textBox_output_code.Text = child.goodsName;
                                outCount = BackGoodsCount(child.outCount);
                                labelMaxCOUNT.Visible = true;
                                labelSell.Visible = true;
                                labelSell.Text = outCount.ToString();
                                goodsCode = child.goodsCode;
                                dataTime_input_maketime.Value = child.makeTime;
                                dateTime_backstorage_date.Value = child.instorageTime;
                                textBox_input_batch_num.Text = child.batchNum;
                                textBox_input_quality_reg.Text = child.qualityReg;
                            }
                        }
                        break;
                    }
        ...
        /// <param name="e"></param>
        /// <history>
        ///    完成信息：吴小科      2010/09/02 完成   
        ///    更新信息：
        /// </history>
        //***********************************************************************
        private int BackGoodsCount(int sourceNum)
        {
            try
            {
                //打开数据库
                ...
                //获取操作类
                GetData getData = new GetData(dataAccess.Connection);
                DataTable sellTable = getData.GetSingleTable("tc_input_storage");
                DataRow[] drs = sellTable.Select("output_code= '" + outputCode + "'");
                if (drs.Length > 0)
                {
                    for (int i = 0; i < drs.Length; i++)
                    {
                        sourceNum -= int.Parse(drs[i]["input_arrival_count"].ToString());
                    }
                }
        ...
        private void dataGridClear()
        {
            ...
            dateTime_backstorage_date.Value = DateTime.Now.Date;
            textBox_input_quality_reg.Text = "";
            textBox_input_remark.Text = "";
        ...
        /// </history>
        //*******************************************************************
        private void BackStorageForm_Load(object sender, EventArgs e)
        {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            sp.SetValue(":tc_input_storage.input_type", '3');
            //判断文本框是否为空
            if (!textBox_customer_name.Text.Trim().Equals(string.Empty))
            {
            ...
            //判断文本框是否为空
            if (!textBox__goods_yxm.Text.Trim().Equals(string.Empty))
            {
                sp.SetValue(":TC_GOODS.GOODS_YXM", textBox__goods_yxm.Text);
            }
            //判断文本框是否为空
            if (!textBox_customer_yxm.Text.Trim().Equals(string.Empty))
            {
            ...
            {
                if (dateTime_input_instorage_date_from.Value.Date > dateTime_input_instorage_date_to.Value.Date)
                {
                    MessageBox.Show("起始日期必须小于终止日期", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    dateTime_input_instorage_date_from.Focus();
                    return;
                }
                //判断时间有效性
                if (DateTime.Now.Date < dateTime_input_instorage_date_to.Value.Date)
                {
                    MessageBox.Show("终止日期必须小于当前日期", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    dateTime_input_instorage_date_to.Focus();
                    return;
                }
                //设置参数赋值
                sp.SetValue(":TC_INPUT_STORAGE.INPUT_INSTORAGE_DATE_FROM", dateTime_input_instorage_date_from.Value.Date);
                sp.SetValue(":TC_INPUT_STORAGE.INPUT_INSTORAGE_DATE_TO", dateTime_input_instorage_date_to.Value.Date);
            }
            try
            {
                ...
                for (int i = 0; i < dtInput_storage.Rows.Count; i++)
                {
                    dtInput_storage.Rows[i]["num"] = ++countNum;

                }
                //绑定数据
                dataGridView_input_storage.DataSource = dtInput_storage;
                if (dtInput_storage.Rows.Count == 0)
                {
                    dataGridClear();
        ...
        private void dataGridView_input_storage_SelectionChanged(object sender, EventArgs e)
        {
            ...
            if (dataGridView_input_storage != null && dataGridView_input_storage.SelectedRows.Count != 0)
            {
                ...
                try
                {
                    outCount = Convert.ToInt32(dataGridView_input_storage.SelectedRows[0].Cells["output_count"].Value.ToString());
                }
                catch (Exception ex)
                {
                    outCount = 0;
                ...
                if (dataGridView_input_storage.SelectedRows[0].Cells["supplier_name"].Value == null)
                {
                    supplierName = "";
                }
                else
                {
                    supplierName = dataGridView_input_storage.SelectedRows[0].Cells["supplier_name"].Value.ToString();
                }//input_arrival_count
                goodsCode = dataGridView_input_storage.SelectedRows[0].Cells["goods_code"].Value.ToString();
                textBox_input_code2.Text = dataGridView_input_storage.SelectedRows[0].Cells["input_code"].Value.ToString();
                textBox_output_code.Text = dataGridView_input_storage.SelectedRows[0].Cells["goods_name"].Value.ToString(); 
                outputCode = dataGridView_input_storage.SelectedRows[0].Cells["output_code"].Value.ToString();
                ...
                dateTime_backstorage_date.Value = DateTime.Parse(dataGridView_input_storage.SelectedRows[0].Cells["backstorage_date"].Value.ToString());
                ...
                textBox_quality_info.Text = Convert.ToString(dataGridView_input_storage.SelectedRows[0].Cells["quality_info"].Value);
                textBox_check_info.Text = Convert.ToString(dataGridView_input_storage.SelectedRows[0].Cells["check_info"].Value);
            }
            dataType = DataType.None;
            setButtonState();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            //设置查询条件参数
            SearchParameter sp = new SearchParameter();
            sp.SetValue(":input_type", '3');
            sp.SetValue(":TC_INPUT_STORAGE.INPUT_CODE", textBox_input_code2.Text);
            //打开数据库
            dataAccess = new DataAccess();
            dataAccess.Open();
            //获取操作类
            ...
            //获取原先数据和当前行号
            oldCount = Convert.ToInt32(dataGridView_input_storage.SelectedRows[0].Cells["input_standard_count"].Value.ToString());
            oldCountArrival = Convert.ToInt32(dataGridView_input_storage.SelectedRows[0].Cells["input_arrival_count"].Value.ToString());
            countNum = dataGridView_input_storage.SelectedRows[0].Index;
            //GetData getData = new GetData(dataAccess.Connection);
            //DataTable sellTable = getData.GetSingleTable("tc_output_storage");
            //DataRow[] dr = sellTable.Select("output_code= '" + outputCode.Text + "'");
            //oldCount = BackGoodsCount(int.Parse(dr[0]["output_count"].ToString())) + oldCount;
            oldCountArrival = SellGoodsCount();
            this.labelMaxCOUNT.Visible = true;
            this.labelSell.Visible = true;
            this.labelSell.Text = oldCountArrival.ToString();
        ...
        private int SellGoodsCount()
        {
            ...
            try
            {
                dataAccess1.Open();
                //获取操作类
                GetData getData = new GetData(dataAccess1.Connection);
                DataTable sellTable = getData.GetSingleTable("tc_output_storage");
                DataRow[] dr = sellTable.Select("output_code= '" + outputCode + "'");
                returnCount = BackGoodsCount(int.Parse(dr[0]["output_count"].ToString())) + oldCountArrival;
            }
            catch (Exception ex)
            {
                MessageBox.Show("请查看数据库是否正常！", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("您确定要删除该数据吗？", this.Text, MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                ...
                //input_storageEntity.INPUT_GOODS_CODE = goodsCode;
                output_storageEntity = new EntityOutput_storage();
                output_storageEntity.Operate_type = "0";
                output_storageEntity.Output_code = outputCode;
                sp3.SetValue(":output_code", output_storageEntity.Output_code);
                //获得表的主健
                string input_code = dataGridView_input_storage.SelectedRows[0].Cells["input_code"].Value.ToString();
                //更新条件参数赋值
                sp1.SetValue(":goods_code", goodsCode);
                sp5.SetValue(":input_code", input_code);
                sp6.SetValue(":disqualification_code", disqualificationCode);
                ...
                try
                {
                    ...
                    GetData getData2 = new GetData(dataAccess1.Connection);
                    DataTable tableTest = getData2.GetSingleTable("tc_apparatus_quality");
                    DataRow[] drs = tableTest.Select("apparatus_output_code='" + outputCode + "'");
                    //取得结果符
                    result1 = getData1.DeleteRow("tc_input_storage", sp);
                    result2 = getData1.UpdateTemp_storage(temp_storageEntity, sp1);
                    result5 = getData1.DeleteRow("tc_disqualification_manage", sp5);
                    result6 = getData1.DeleteRow("tc_disqualification_to", sp6);
                    if (drs.Length == 0)
                    {
                        result3 = getData1.UpdateOutput_storage(output_storageEntity, sp3);
                    }
                    //提交事务
                    dataAccess.Commit();
                    //出现错误，提示信息
        ...
        private void btnCommit_Click(object sender, EventArgs e)
        {
            ...
            //入库类型标记
            input_storageEntity.INPUT_TYPE = '3';
            //判断该字段是否为空   
            if (textBox_output_code.Text.Equals("双击选择一个产品名称..."))
            {
            ...
            {
                input_storageEntity.OUTPUT_CODE = outputCode;
            }
            //判断时间有效性
            //if (dataTime_input_maketime.Value.Date > dateTime_input_instorage_date.Value.Date)
            //{
            //    MessageBox.Show("入库日期不能小于生产日期", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
            //    dateTime_input_instorage_date.Focus();
            //    return;
            //}
            //判断时间有效性
            if (dateTime_input_instorage_date.Value.Date < dateTime_backstorage_date.Value.Date)
            {
                MessageBox.Show("入库日期不能小于退货日期", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                dateTime_input_instorage_date.Focus();
                return;
            }
            if (dateTimePicker_CheckTime.Value.Date < dateTime_input_instorage_date.Value.Date)
            {
                MessageBox.Show("验收日期不能小于入库日期", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                dateTimePicker_CheckTime.Focus();
                return;
            }
            //判断时间有效性
            if (DateTime.Now.Date < dateTimePicker_CheckTime.Value.Date)
            {
            ...
            //判断该字段是否为空且是否为正确的数字格式
            if (Convert.ToInt32(textBox_input_arrival_count.Value) == 0)
            {
                MessageBox.Show("退货数量不能为0，请重新输入！", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                textBox_input_arrival_count.Focus();
                return;
            }

            if (Convert.ToInt32(textBox_input_arrival_count.Value) > Convert.ToInt32(labelSell.Text))
            {
                MessageBox.Show("退货数量不能大于限制数量，请核对后重新输入！", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                textBox_input_arrival_count.Focus();
                return;
            }
            if (textBox_input_arrival_count.Text == "")
            {
                MessageBox.Show("退货数量不能为空，请重新输入！", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                textBox_input_arrival_count.Focus();
                return;

            }
            if (Convert.ToInt32(textBox_input_standard_count.Value) > Convert.ToInt32(textBox_input_arrival_count.Value))
            {
                MessageBox.Show("合格数量不能大于退货数量，请核对后重新输入！", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
                textBox_input_arrival_count.Focus();
                return;
            }
            ...
            input_storageEntity.BACKSTORAGE_DATE = dateTime_backstorage_date.Value.Date;
            input_storageEntity.INPUT_CHECKTIME = dateTimePicker_CheckTime.Value.Date;
            ...
            // input_storageEntity.SUPPLIER_NAME = supplierName;
            ...
            if (dataType == DataType.Insert)
            {
                ...
                input_storageEntity.INPUT_GOODS_CODE = goodsCode;
                output_storageEntity = new EntityOutput_storage();
                output_storageEntity.Operate_type = "1";
                output_storageEntity.Output_code = outputCode;
                sp1.SetValue(":output_code", output_storageEntity.Output_code);
                //取得实体类
                temp_storageEntity = new EntityTemp_storage();
                ...
                sp.Clear();
                //更新条件参数赋值
                sp.SetValue(":goods_code", goodsCode);
                disCount = input_storageEntity.INPUT_ARRIVAL_COUNT - input_storageEntity.INPUT_STANDARD_COUNT;
                if (disCount > 0)
                {
                    DialogResult result = MessageBox.Show("合格数量与退货数量不一致，其中不合格产品有：" + disCount.ToString() + "，您确定添加吗？ ", this.Text, MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                ...
                try
                {
                    ...
                    result3 = getData.UpdateOutput_storage(output_storageEntity, sp1);
                    if (disCount != 0)
                    {
                        getData.InsertDisqualification_Manager(disqualificationEntity);
                    }
                    if (disCount == 0)
                    {
                        editFlag = 1;
            ...
            else if (dataType == DataType.Update)
            {
                ...
                disqualificationEntity = new EntityDisqualification();
                //赋值实体类
                temp_storageEntity.Count = Convert.ToInt32(textBox_input_standard_count.Value) - oldCount;
                //设置更新条件参数
                disCount = input_storageEntity.INPUT_ARRIVAL_COUNT - input_storageEntity.INPUT_STANDARD_COUNT;
                if (disCount > 0)
                {
                ...
                try
                {
                    ...
                    //取得结果符
                    result1 = getData1.UpdateInput_storageRow2(input_storageEntity, sp1);
                ...
                catch (Exception ex)
                {
                    ...
                    //提示错误
                    MessageBox.Show("数据修改时发生错误，请检查数据库！", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void checkBox1_CheckedChanged(object sender, EventArgs e)
        {
            if (checkBox1.Checked == true)
            {
                dateTime_input_instorage_date_from.Enabled = true;
                dateTime_input_instorage_date_to.Enabled = true;
            }
